...
describe('Hacker Shop <App/>', () => {
  ...
  it('Check Shipping cost (free shipping if Subtotal > $50)', () => {
    ...
    expect(wrapper.find('tr#shippingRow').text()).toEqual('Shipping$ 0');
    expect(wrapper.find('tr#taxRow').text()).toEqual('Tax(5%)$ 4');
  });
  ...
  it('Add item multiple times and deleting the item', () => {
    ...
    wrapper.find('button#buttonAdd3').simulate('click');
    wrapper.find('button#buttonAdd3').simulate('click');
    expect(wrapper.find('tr#itemRow0').text()).toEqual('Shoe3$ 150');
    wrapper.find('button#buttonDelete3').simulate('click');
    expect(wrapper.find('tr#itemRow0').text()).toEqual('Shoe2$ 100');
    wrapper.find('button#buttonDelete3').simulate('click');
    expect(wrapper.find('tr#itemRow0').text()).toEqual('Shoe1$ 50');
  });
